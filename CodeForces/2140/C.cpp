...
void solve()
{
	...
	if(n==2)
	{
		ans=max(ans,a[2]-a[1]+1);
		cout<<ans<<endl;
		return;
	}
//	DEBUG(ans,2);
	// r-2a[r]-l+2a[l],l%2=0,r%2=1
	LL lpart=-INF;
	for(int i=1;i<=n;i++)
	{
		if(i%2==1)
		{
			ans=max(ans,sum+i-2*a[i]+lpart);
		}
		else
		{
			lpart=max(lpart,-i+2*a[i]);
		}
	}
//	DEBUG(ans,2);
	// r+2a[r]-l-2a[l],l%2=1,r%2=0
	lpart=-INF;
	for(int i=1;i<=n;i++)
	{
		if(i%2==0)
